...
if(Modules == nil) then
	...
	function StdModule.say(cid, message, keywords, parameters, node)
		...
		local onlyFocus = (parameters.onlyFocus == nil or parameters.onlyFocus)
	...
	function StdModule.promotePlayer(cid, message, keywords, parameters, node)
		...
		if(isPlayerPremiumCallback == nil or isPlayerPremiumCallback(cid) or not parameters.premium) then
			...
			if(getPlayerStorageValue(cid, 30018)) then
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if(inBackpacks) then
			totalCost = isItemStackable(itemid) and totalCost + 20 or totalCost + (math.max(1, math.floor(amount / getContainerCapById(backpack))) * 20)
		end
