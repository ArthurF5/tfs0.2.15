...
#endif
{
	...
	if(cleanInterval > 0)
	{
		...
		if(cleanWarningInterval > 0 && cleanWarningInterval < cleanInterval)
			g_scheduler.addEvent(createSchedulerTask((cleanInterval * 60000) - (cleanWarningInterval * 60000), boost::bind(&Game::cleanServer, &g_game, cleanWarningInterval, true)));
		else
			g_scheduler.addEvent(createSchedulerTask(cleanInterval * 60000, boost::bind(&Game::cleanServer, &g_game, 0, true)));
	} 
