...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	if(readXMLInteger(p, "blackCanUse", intValue))
		blackCanUse = (intValue == 1);
...
bool Spell::playerSpellCheck(Player* player, bool ignoreExhaust/* = false*/) const
{
	...
	if(!player->hasFlag(PlayerFlag_IgnoreSpellCheck))
	{
		...
		if(!blackCanUse && player->getSkull() == SKULL_BLACK)
		{
...
bool Spell::playerInstantSpellCheck(Player* player, const Position& toPos)
{
	if(toPos.x != 0xFFFF)
	{
		...
		else
		{
			...
			{
				if(isAggressive && needTarget && tile->getTopVisibleCreature(player))
				{
					Player* targetPlayer = tile->getTopVisibleCreature(player)->getPlayer();
					if(targetPlayer && targetPlayer != player && player->getSkullClient(targetPlayer->getPlayer()) == SKULL_NONE)
					{
