...
bool ConfigManager::loadFile(const std::string& _filename)
{
	...
	m_confBoolean[ANIMATION_TEXT_ON_HEAL] = booleanString(getGlobalString(L, "animationTextOnHeal", "yes"));
	...
	m_confBoolean[CLEAN_PROTECTED_ZONES] = booleanString(getGlobalString(L, "cleanProtectedZones", "yes"));
	...
	m_confInteger[AUTO_CLEAN_EACH_MINUTES] = getGlobalNumber(L, "autoCleanEachMinutes", 0);
	m_confInteger[CLEAN_WARNING_BEFORE] = getGlobalNumber(L, "cleanWarningBefore", 0);
	m_confInteger[TILE_LIMIT] = getGlobalNumber(L, "tileLimit", 0);
	m_confInteger[PROTECTION_TILE_LIMIT] = getGlobalNumber(L, "protectionTileLimit", 0);
	m_confInteger[HOUSE_TILE_LIMIT] = getGlobalNumber(L, "houseTileLimit", 0);
